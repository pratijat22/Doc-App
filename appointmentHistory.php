...
{   
    ...
    elseif ($_SESSION['role'] == 'patient')
        header('location:myAppointmengtsP.php');
}
else
{
    session_destroy();
...
<!-- Breadcrumb -->
    ...
    ?>
        ...
        <li class="breadcrumb-item active" aria-current="page">My Appointment History</li>    
    </ol>
  </nav>
...
<?php 
    ...
    $date = date('Y-m-d');
    $doc_id = $_SESSION['id'];
    $sql = " SELECT * FROM booking_request WHERE d_id = '$doc_id' ";
	$result = mysqli_query($connect,$sql);
	$count = mysqli_num_rows($result); 
?>

  <!-- My Appointments Details Table if avilable -->
  <?php 
  if($count>=1){ 
?>
...
<?php }else{
?>
  <!-- My Appointments Details Table if not avilable -->
